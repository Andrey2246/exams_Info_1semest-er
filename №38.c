...
void sorting(int *arr, int n, int *ans){
    for(int i = 0; i < n; i++){ // считаем элементы массива, строго меньшие текущего
        int count = 0;
        for(int j = 0; j < n;j++){
            if (arr[j] < arr[i]){
                count++;
        ...
        ans[count]=arr[i];  //  ставим текущие элемент на свое место в выходном массиве(наименьший на [0])
    }
    for(int j = 1; j < n;j++){ //  меняем -1 на предыдущий элемент в выходном массиве
        if (ans[j] == -1){
            ans[j] = ans[j - 1];
        }
    }
